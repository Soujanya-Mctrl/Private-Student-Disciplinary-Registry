...
import { Contract, ledger as disciplinaryLedger } from './managed/disciplinary/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'disciplinary');
...
// Types
// ---------------------------------------------------------------------------
type DisciplinaryPrivateState = { privateRecord: Record<string, bigint> };
type DisciplinaryCircuits = ImpureCircuitId<Contract<DisciplinaryPrivateState>>;
...
async function main() {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<DisciplinaryCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof DisciplinaryPrivateStateId>({
      privateStateStoreName: 'disciplinary-private-state',
  ...
  // --- Compile contract ---
  const disciplinaryCompiledContract = CompiledContract.make('disciplinary', Contract).pipe(
  ...
  const contract = await withStatus('Deploying counter contract (this may take a few minutes)', async () => {
    return deployContract(providers, {
      ...
      initialPrivateState: { privateRecord: {} },
    });
  });
